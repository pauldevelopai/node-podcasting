# install.ps1 — one-command installer for the Podcast Studio Node (Windows).
#
...
$Repo    = if ($env:GROUNDED_REPO)         { $env:GROUNDED_REPO }         else { 'pauldevelopai/node-podcasting' }
...
$AppDir  = Join-Path $Root 'node-podcasting'
$Name    = 'Podcast Studio'
